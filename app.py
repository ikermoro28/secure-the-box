...
class SecureTheBoxApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # --- AHORA SON 3 COLUMNAS PARA QUE SEAN MÁS PEQUEÑAS ---
        ...
        self.frame_lista.grid_columnconfigure(2, weight=1, uniform="grupo1")
    ...
    def cargar_lista_retos(self):
        try:
            ...
            def calcular_peso_dificultad(reto):
                dif = str(reto[2]).lower()
                if dif in ['1', 'fácil', 'facil']: return 1
                elif dif in ['2', 'media', 'medio']: return 2
                elif dif in ['3', 'difícil', 'dificil']: return 3
                return 4
            ...
            for index, reto in enumerate(retos):
                ...
                dif = str(dif)
                texto_dif = "FÁCIL" if dif.lower() in ['1', 'fácil'] else "MEDIA" if dif.lower() in ['2', 'media'] else "DIFÍCIL" if dif.lower() in ['3', 'difícil'] else dif.upper()
                color_dif = DIFICULTAD_COLORS.get(dif.lower(), '#cccccc') 

                # TARJETA: Borde más grueso (border_width=2)
                card = ctk.CTkFrame(self.frame_lista, fg_color="#1a1a1a", corner_radius=10, border_width=3, border_color="#333333")
                ...
                # FOOTER: Contendrá Dificultad a la Izquierda y Puntos a la Derecha
